...
static QDate nthWeekdayOfMonth(int year, int month, Qt::DayOfWeek weekday,
                               int n) {
...
static QList<CalendarEvent> generateUSHolidays(int year) {
  ...
  add(nthWeekdayOfMonth(year, 1, Qt::Monday, 3),
      QObject::tr("Martin Luther King Jr. Day"));
  add(nthWeekdayOfMonth(year, 2, Qt::Monday, 3),
      QObject::tr("Washington's Birthday"));
  ...
  add(nthWeekdayOfMonth(year, 11, Qt::Thursday, 4),
      QObject::tr("Thanksgiving Day"));
...
CyberDom::CyberDom(QWidget *parent)
  ...
  connect(ui->resetTimer, &QPushButton::clicked, this,
          &CyberDom::onResetSigninTimer);
  ui->timerLabel->hide();
  ui->resetTimer->hide();
  ui->lbl_Timer->hide();
  ...
  connect(ui->actionAssignments, &QAction::triggered, this,
          &CyberDom::openAssignmentsWindow);

  // Connect the Time_Add action to a slot function
  connect(ui->actionTimeAdd, &QAction::triggered, this,
          &CyberDom::openTimeAddDialog);

  // Connect the About action to a slot function
  connect(ui->actionAbout, &QAction::triggered, this,
          &CyberDom::openAboutDialog);

  // Connect the AskClothing action to a slot function
  connect(ui->actionAsk_for_Clothing_Instructions, &QAction::triggered, this,
          [this]() { openAskClothingDialog(); });

  // Connect the AskInstructions action to a slot function
  connect(ui->actionAsk_for_Other_Instructions, &QAction::triggered, this,
          [this]() { openAskInstructionsDialog(); });

  // Connect the ReportClothing action to a slot function
  connect(ui->actionReport_Clothing, &QAction::triggered, this,
          [this]() { openReportClothingDialog(); });

  // Connect the AskPunishment action to a slot function
  connect(ui->actionAsk_for_Punishment, &QAction::triggered, this,
          &CyberDom::openAskPunishmentDialog);

  // Connect the Change_Merit_Points action to a slot function
  connect(ui->actionChange_Merit_Points, &QAction::triggered, this,
          &CyberDom::openChangeMeritsDialog);

  // Connect the Change_Status action to a slot function
  connect(ui->actionStatus_Change, &QAction::triggered, this,
          &CyberDom::openChangeStatusDialog);

  // Connect the Select_Job action to a slot function
  connect(ui->actionJob_Launch, &QAction::triggered, this,
          &CyberDom::openLaunchJobDialog);
  connect(ui->actionCalendar, &QAction::triggered, this,
          &CyberDom::openCalendarView);
  ...
  connect(ui->actionPunish, &QAction::triggered, this,
          &CyberDom::openSelectPunishmentDialog);

  // Connect the Select_Popup action to a slot function
  connect(ui->actionPop_Up_Launch, &QAction::triggered, this,
          &CyberDom::openSelectPopupDialog);

  // Connect the List_Flags action to a slot function
  connect(ui->actionList_and_Remove_Flags, &QAction::triggered, this,
          &CyberDom::openListFlagsDialog);

  // Connect the SetFlags action to a slot function
  connect(ui->actionSet_Flags, &QAction::triggered, this,
          &CyberDom::openSetFlagsDialog);

  // Connect the DeleteAssignments action to a slot function
  connect(ui->actionList_and_Delete_Assignments, &QAction::triggered, this,
          &CyberDom::openDeleteAssignmentsDialog);

  // Connect the Delete_Status_File action to a slot function
  connect(ui->actionDelete_Status_File, &QAction::triggered, this,
          &CyberDom::resetApplication);

  // Connect the viewScriptData action to a slot function
  connect(ui->actionViewScriptData, &QAction::triggered, this,
          &CyberDom::openDataInspector);

  // Connect the Make Report File action
  connect(ui->actionMake_A_New_Report_File, &QAction::triggered, this,
          &CyberDom::onMakeReportFile);
  connect(ui->actionView_Report_File, &QAction::triggered, this,
          &CyberDom::onViewReportFile);
  ...
  QString oldSessionPath =
      QCoreApplication::applicationDirPath() + "/session.cdt";
  ...
  QString settingsPath =
      QCoreApplication::applicationDirPath() + "/cyberdom_settings.ini";
...
void CyberDom::setupCamera() {
  ...
  if (defaultCamera.isNull()) {
    qDebug()
        << "[Camera] No camera device found. Camera features will be disabled.";
...
void CyberDom::takePicture(const QString &filenamePrefix) {
  ...
  if (!camera || !imageCapture || !imageCapture->isReadyForCapture()) {
    qDebug() << "[Camera] Cannot take picture. Camera is not initialized or "
                "not ready.";
    return;
  }
  ...
  QString fileName =
      QString("%1/%2_%3.jpg").arg(folder, filenamePrefix, timestamp);
...
void CyberDom::onImageCaptureError(int id, QImageCapture::Error error,
...
CyberDom::~CyberDom() {
  ...
  if (!settingsFile.isEmpty()) {
    ...
    settings.setValue("User/LastCloseTime",
...
int CyberDom::getMinMerits() const { return minMerits; }

int CyberDom::getMaxMerits() const { return maxMerits; }

QString CyberDom::getAskPunishmentGroups() const { return askPunishmentGroups; }

int CyberDom::getAskPunishmentMin() const { return askPunishmentMin; }

int CyberDom::getAskPunishmentMax() const { return askPunishmentMax; }

void CyberDom::openAssignmentsWindow() {
  if (checkInterruptableAssignments())
    return;

  if (!assignmentsWindow) {
    assignmentsWindow = new Assignments(this, this);
    connect(this, &CyberDom::jobListUpdated, assignmentsWindow,
            &Assignments::populateJobList);
  }
...
// {
//     if (!assignmentsWindow) {
//         assignmentsWindow = new QMainWindow(this); // Create the window if
//         not already created assignmentsUi.setupUi(assignmentsWindow); //
//         Setup the Assignments UI
//     }
...
void CyberDom::openTimeAddDialog() {
  ...
  TimeAdd timeAddDialog(
      this); // Pass the parent to ensure proper memory management
  connect(&timeAddDialog, &TimeAdd::timeAdded, this,
          &CyberDom::applyTimeToClock); // Connect the timeAdded signal to the
                                        // applyTimeToClock slot
...
void CyberDom::applyTimeToClock(int days, int hours, int minutes, int seconds) {
  ...
  internalClock = internalClock.addDays(days).addSecs(hours * 3600 +
                                                      minutes * 60 + seconds);
...
void CyberDom::updateInternalClock() {
  ...
  if (previousTime.date() != internalClock.date()) {
    qDebug() << "Date changed from: " << previousTime.toString("MM-dd-yyyy")
             << "to: " << internalClock.toString("MM-dd-yyyy");
    ...
    if (scriptParser) {
      QString newDayProcedure =
          scriptParser->getScriptData().eventHandlers.newDay;
    ...
    if (!timer.triggered && now >= timer.start && now <= timer.end) {
      if (scriptParser) {
        ...
        if (defs.contains(timer.name)) {
          ...
          if (!isTimeAllowed(td.notBeforeTimes, td.notAfterTimes,
                             td.notBetweenTimes))
            continue;
        }
      }
        ...
        if (defs.contains(timer.name)) {
          ...
          for (const ScriptAction &action : td.actions) {
            if (skipNextAction) {
              ...
              if (action.type != ScriptActionType::If &&
                  action.type != ScriptActionType::NotIf) {
                continue;
              }
            }
            ...
            case ScriptActionType::Message:
              QMessageBox::information(this, tr("Timer"),
            ...
            default:
              qDebug() << "[WARN] Unhandled action type in permission:"
...
void CyberDom::openAskClothingDialog(const QString &target) {
  ...
  if (!target.isEmpty()) {
    ...
    // (This logic ensures we don't re-roll random choices and get a different
    // result)
  }
  ...
  if (!preCalculatedText.isEmpty()) {
    ...
    // If no target (user menu), trigger the selection manually to show the
    // first item's text
...
void CyberDom::openReportClothingDialog(bool forced, const QString &title) {
  if (!forced && checkInterruptableAssignments())
    return;

  // Pass the new parameters to the dialog
  ReportClothing *reportClothingDialog =
      new ReportClothing(this, scriptParser, forced, title);

  if (reportClothingDialog->exec() == QDialog::Accepted) {
    processClothingReport(reportClothingDialog->getWearingItems(),
                          reportClothingDialog->isNaked());
  }
...
void CyberDom::openReport(const QString &name) {
  ...
  if (reports.contains(name)) {
    ...
    if (!isTimeAllowed(rep.notBeforeTimes, rep.notAfterTimes,
                       rep.notBetweenTimes)) {
      QMessageBox::information(
          this, tr("Report"), tr("This report is not available at this time."));
      return;
    }
  }
...
void CyberDom::openAskPermissionDialog() {
  ...
  bool ok = false;
  QString choice =
      QInputDialog::getItem(this, tr("Ask Permission"),
...
void CyberDom::openPermission(const QString &name) {
  ...
  if (granted) {
    ...
    if (msg.isEmpty())
      msg = tr("Yes %1, you may %2.")
                .arg(subName, perm.title.isEmpty() ? name : perm.title);
    QMessageBox::information(this, tr("Permission"),
                             replaceVariables(msg, name));

    // Global Event
    QString eventProc =
        scriptParser->getScriptData().eventHandlers.permissionGiven;
    if (!eventProc.isEmpty())
      runProcedure(eventProc);
    ...
    currentMerits =
        std::max(getMinMerits(), std::min(getMaxMerits(), currentMerits));
    updateMerits(currentMerits);

    // Execute Actions
    qDebug() << "[Permission] Grant executing" << perm.actions.size()
             << "actions for" << name;
    ...
    for (const ScriptAction &action : perm.actions) {
      ...
      case ScriptActionType::NewSubStatus:
        qDebug() << "[Permission] Grant executing NewSubStatus:"
                 << action.value;
    ...
    if (delaySecs > 0) {
      if (perm.notify == 1) {
        ...
        msg +=
            "\n\n" +
            tr("You must wait %1. You will be notified when you can ask again.")
                .arg(formatDuration(delaySecs));
        ...
        // Notify=0 (Default): Just tell them to wait (or say nothing extra if
        // you prefer) Standard behavior is usually to give a hint unless hidden
    ...
    if (!perm.denyStatus.isEmpty()) {
      qDebug() << "[Permission] Denied executing DenyStatus:"
               << perm.denyStatus;
...
bool CyberDom::isPermissionForbidden(const QString &name) const {
  ...
  for (const QString &assignmentName : activeAssignments) {
    ...
    const PunishmentDefinition *punDef =
        getPunishmentDefinition(assignmentName);
    if (!punDef)
      continue;
...
void CyberDom::openConfession(const QString &name) {
  ...
  // --- 2. Check Time ---
  if (!isTimeAllowed(conf.notBeforeTimes, conf.notAfterTimes,
                     conf.notBetweenTimes)) {
    QMessageBox::information(
        this, tr("Confession"),
...
void CyberDom::populateReportMenu() {
  ...
  for (auto it = reports.constBegin(); it != reports.constEnd(); ++it) {
    ...
    connect(act, &QAction::triggered, this,
            [this, name = rep.name]() { openReport(name); });
  }
}
...
void CyberDom::populateConfessMenu() {
  ...
  for (const auto &conf : confessions) {
    ...
    connect(act, &QAction::triggered, this,
            [this, name = conf.name]() { openConfession(name); });
  }
}
...
void CyberDom::populatePermissionMenu() {
  ...
  for (const auto &perm : perms) {
    ...
    connect(act, &QAction::triggered, this,
            [this, name = perm.name]() { openPermission(name); });
  }
}
...
void CyberDom::setupMenuConnections() {
  ...
  connect(ui->actionPermissions, &QAction::triggered, this,
          [this]() { rulesDialog->updateRulesDialog(Permissions); });

  connect(ui->actionReports, &QAction::triggered, this,
          [this]() { rulesDialog->updateRulesDialog(Reports); });

  connect(ui->actionConfessions, &QAction::triggered, this,
          [this]() { rulesDialog->updateRulesDialog(Confessions); });

  connect(ui->actionClothing, &QAction::triggered, this,
          [this]() { rulesDialog->updateRulesDialog(Clothing); });

  connect(ui->actionInstructions, &QAction::triggered, this,
          [this]() { rulesDialog->updateRulesDialog(Instructions); });

  connect(ui->actionOtherRules, &QAction::triggered, this,
          [this]() { rulesDialog->updateRulesDialog(OtherRules); });

  connect(ui->actionExport_Clothes, &QAction::triggered, this,
          &CyberDom::exportClothes);
...
void CyberDom::openAskPunishmentDialog() {
  ...
  if (maxPunishment <= 0) {
    QMessageBox::information(this, "Not Available",
                             "You cannot ask for a punishment right now.");
    return;
  }
  ...
  if (askPunishmentDialog.exec() == QDialog::Accepted) {
    ...
    if (minPunishment == maxPunishment) {
      ...
      severity =
          ScriptUtils::randomInRange(minPunishment, maxPunishment, false);
    }
    ...
    if (scriptParser) {
      QString eventProc =
          scriptParser->getScriptData().eventHandlers.punishmentAsked;
      if (!eventProc.isEmpty()) {
        runProcedure(eventProc);
      }
    }
    ...
    qDebug()
        << "[AskPunishment] User requested punishment. Calculated Severity:"
        << severity << "Groups:" << groups;
...
void CyberDom::openChangeMeritsDialog() {
  ...
  connect(&changeMeritsDialog, &ChangeMerits::meritsUpdated, this,
          &CyberDom::updateMerits);
...
void CyberDom::openChangeStatusDialog() {
  ...
  if (scriptParser) {
    ...
    qDebug() << "[DEBUG] Retrieved " << availableStatuses.size()
             << " statuses directly from ScriptParser";
  } else {
  ...
  if (availableStatuses.isEmpty()) {
    qDebug()
        << "[ERROR] No statuses found in the script (or scriptParser is null)";
    QMessageBox::critical(this, "Error",
                          "No statuses could be loaded from the script.");
    return;
  }
  ...
  connect(&changeStatusDialog, &ChangeStatus::statusUpdated, this,
          &CyberDom::updateStatus);
...
void CyberDom::openLaunchJobDialog() {
  ...
  JobLaunch launchJobDialog(
      this);              // Create the SelectJob dialog, passing the parent
...
void CyberDom::openListFlagsDialog() {
  ...
  ListFlags listFlagsDialog(
      this, &flags); // Create the ListFlags dialog, passing the parent
  connect(&listFlagsDialog, &ListFlags::flagRemoveRequested, this,
          &CyberDom::removeFlag);
...
void CyberDom::openSetFlagsDialog() {
  ...
  connect(&setFlagsDialog, &SetFlags::flagSetRequested, this,
          &CyberDom::setFlag);
  setFlagsDialog.exec();
}
...
void CyberDom::openDeleteAssignmentsDialog() {
  ...
  DeleteAssignments deleteAssignmentsDialog(
      this); // Create the DeleteAssignments dialog, passing the parent
...
QString CyberDom::promptForIniFile() {
  ...
  QString filePath = QFileDialog::getOpenFileName(
      this, tr("Select CyberDom Script File"), startDir,
      tr("Script Files (*.ini);;All Files (*)"));

  if (filePath.isEmpty()) {
    QMessageBox::warning(this, "No File Selected",
                         "You need to select a valid script file (.ini) to "
                         "proceed with CyberDom.");
  }
...
void CyberDom::initializeIniFile() {
  ...
  if (!iniFilePath.isEmpty() && !QFile::exists(iniFilePath)) {
    QMessageBox::warning(this, tr("Script Not Found"),
                         tr("The previously selected script could not be "
                            "found. Please locate it again."));
  ...
  // If this is a fresh start or no ini file path is stored, prompt for a new
  // file
  ...
  qDebug() << "[DEBUG] File size: " << QFileInfo(iniFilePath).size()
           << " bytes";
...
void CyberDom::updateStatus(const QString &newStatus) {
  ...
  if (scriptParser) {
    QString proc =
        scriptParser->getScriptData().eventHandlers.beforeStatusChange;
    if (!proc.isEmpty())
      runProcedure(proc);
  }
  ...
  if (scriptParser) {
    QString proc =
        scriptParser->getScriptData().eventHandlers.afterStatusChange;
    if (!proc.isEmpty())
      runProcedure(proc);
  }

  // Check Camera Status
  QString camText = status.pointCameraText.isEmpty() ? status.pointCameraText
                                                     : status.pointCameraText;
...
void CyberDom::updateStatusText() {
  ...
  for (const QString &assignmentName : activeAssignments) {
    ...
    const PunishmentDefinition *ptrDef =
        getPunishmentDefinition(assignmentName);
    bool isPun = (ptrDef != nullptr);

    if (!isPun && !jobDefs.contains(lowerName))
      continue;

    QString startFlagName =
        (isPun ? "punishment_" : "job_") + assignmentName + "_started";
    ...
    if (isFlagSet(startFlagName)) {
      ...
      if (isPun) {
        ...
        if (unit == "day" || unit == "hour" || unit == "minute") {
          ...
          qint64 totalSecs = 0;
          if (unit == "day")
            totalSecs = (qint64)(amount * val * 86400);
          else if (unit == "hour")
            totalSecs = (qint64)(amount * val * 3600);
          else if (unit == "minute")
            totalSecs = (qint64)(amount * val * 60);

          if (totalSecs > 0)
            minTimeStr = ScriptUtils::formatDurationString((int)totalSecs);
          else
      ...
      // Gather Timings
      QDateTime start =
          settings.value("Assignments/" + assignmentName + "_start_time")
              .toDateTime();
      QDateTime creation =
          settings.value("Assignments/" + assignmentName + "_creation_time")
              .toDateTime();
      ...
      for (QString line : rawLines) {
        ...
        line.replace("{!zzRunTime}", runTimeStr,
                     Qt::CaseInsensitive); // Force correct runtime
...
void CyberDom::setFlag(const QString &flagName, int durationMinutes) {
  ...
  if (scriptParser) {
    ...
    if (flagDefs.contains(lowerFlagName)) {
      ...
      if (!flagDef.durationMax.isEmpty() &&
          flagDef.durationMax != flagDef.durationMin) {
      ...
      if (!flagDef.group.isEmpty()) {
        const QStringList rawGroups =
            flagDef.group.split(',', Qt::SkipEmptyParts);
...
void CyberDom::removeFlag(const QString &flagName) {
  ...
  if (flags.contains(targetKey)) {
    ...
    qDebug() << "[Flag] Warning: Attempted to remove non-existent flag:"
             << flagName;
  }
}
...
QString CyberDom::replaceVariables(
    const QString &input, const QString &contextName,
    const QString &contextTitle, int contextSeverity,
    const QString &contextMinTime, const QString &contextMaxTime,
    const QDateTime &contextStartTime, const QDateTime &contextCreationTime,
    const QDateTime &contextDeadline, const QDateTime &contextNextRemind,
  ...
  if (!creationTime.isValid() && !contextName.isEmpty()) {
    QSettings settings(settingsFile, QSettings::IniFormat);
    QVariant val =
        settings.value("Assignments/" + contextName + "_creation_time");
    if (val.isValid()) {
  ...
  if (result.contains("zzName", Qt::CaseInsensitive)) {
    ...
    QString val = ScriptUtils::resolvePredefinedString(
        "zzName", data, internalClock, lastReportFilename, APP_VERSION,
        contextName, contextTitle, lastReportName, lastPermissionName,
        lastQuestionAnswer);
    QRegularExpression nameRx("\\[!$]zzName\\}|\\$zzName",
  ...
  if (result.contains("zzTitle", Qt::CaseInsensitive)) {
    QString val = ScriptUtils::resolvePredefinedString(
        "zzTitle", data, internalClock, lastReportFilename, APP_VERSION,
        contextName, contextTitle, lastReportName, lastPermissionName,
        lastQuestionAnswer);
    QRegularExpression titleRx("\\{[!$]zzTitle\\}|$zzTitle",
                               QRegularExpression::CaseInsensitiveOption);
  ...
  if (result.contains("zzSubName", Qt::CaseInsensitive)) {
    QString val = ScriptUtils::resolvePredefinedString(
        "zzSubName", data, internalClock, lastReportFilename, APP_VERSION,
        contextName, contextTitle, lastReportName, lastPermissionName,
        lastQuestionAnswer);
    // Replace {$zzSubName}, {!zzSubName}, and $zzSubName
    QRegularExpression subRx("\\{[!$]zzSubName\\}|\\$zzSubName",
                             QRegularExpression::CaseInsensitiveOption);
  ...
  if (result.contains("zzMaster", Qt::CaseInsensitive)) {
    QString val = ScriptUtils::resolvePredefinedString(
        "zzMaster", data, internalClock, lastReportFilename, APP_VERSION,
        contextName, contextTitle, lastReportName, lastPermissionName,
        lastQuestionAnswer);
    QRegularExpression masterRx("\\{[!$]zzMaster\\}|\\$zzMaster",
                                QRegularExpression::CaseInsensitiveOption);
  ...
  if (result.contains("zzDate", Qt::CaseInsensitive)) {
    QString val = ScriptUtils::resolvePredefinedString(
        "zzDate", data, internalClock, lastReportFilename, APP_VERSION,
        contextName, contextTitle, lastReportName, lastPermissionName,
        lastQuestionAnswer);
    QRegularExpression dateRx("\\{[!$]zzDate\\}|\\$zzDate",
                              QRegularExpression::CaseInsensitiveOption);
  ...
  if (result.contains("zzTime", Qt::CaseInsensitive)) {
    QString val = ScriptUtils::resolvePredefinedString(
        "zzTime", data, internalClock, lastReportFilename, APP_VERSION,
        contextName, contextTitle, lastReportName, lastPermissionName,
        lastQuestionAnswer);
    QRegularExpression timeRx("\\{[!$]zzTime\\}|\\$zzTime",
                              QRegularExpression::CaseInsensitiveOption);
  ...
  if (result.contains("zzDayOfWeek", Qt::CaseInsensitive)) {
    QString val = ScriptUtils::resolvePredefinedString(
        "zzDayOfWeek", data, internalClock, lastReportFilename, APP_VERSION,
        contextName, contextTitle, lastReportName, lastPermissionName,
        lastQuestionAnswer);
    QRegularExpression dayRx("\\{[!$]zzDayOfWeek\\}|\\$zzDayOfWeek",
                             QRegularExpression::CaseInsensitiveOption);
  ...
  if (result.contains("zzMonth", Qt::CaseInsensitive)) {
    QString val = ScriptUtils::resolvePredefinedString(
        "zzMonth", data, internalClock, lastReportFilename, APP_VERSION,
        contextName, contextTitle, lastReportName, lastPermissionName,
        lastQuestionAnswer);
    QRegularExpression monthRx("\\{[!$]zzMonth\\}|\\$zzMonth",
                               QRegularExpression::CaseInsensitiveOption);
  ...
  if (result.contains("zzReportFile", Qt::CaseInsensitive)) {
    QString val = ScriptUtils::resolvePredefinedString(
        "zzReportFile", data, internalClock, lastReportFilename, APP_VERSION,
        contextName, contextTitle, lastReportName, lastPermissionName,
        lastQuestionAnswer);
    QRegularExpression reportRx("\\{[!$]zzReportFile\\}|\\$zzReportFile",
                                QRegularExpression::CaseInsensitiveOption);
  ...
  if (result.contains("zzPVersion", Qt::CaseInsensitive)) {
    QString val = ScriptUtils::resolvePredefinedString(
        "zzPVersion", data, internalClock, lastReportFilename, APP_VERSION,
        contextName, contextTitle, lastReportName, lastPermissionName,
        lastQuestionAnswer);
    QRegularExpression verRx("\\{[!$]zzPVersion\\}|\\$zzPVersion",
                             QRegularExpression::CaseInsensitiveOption);
  ...
  if (result.contains("zzSVersion", Qt::CaseInsensitive)) {
    QString val = ScriptUtils::resolvePredefinedString(
        "zzSVersion", data, internalClock, lastReportFilename, APP_VERSION,
        contextName, contextTitle, lastReportName, lastPermissionName,
        lastQuestionAnswer);
    QRegularExpression scriptVerRx("\\{[!$]zzSVersion\\}|\\$zzSVersion",
                                   QRegularExpression::CaseInsensitiveOption);
  ...
  if (result.contains("zzReport", Qt::CaseInsensitive)) {
    QString val = ScriptUtils::resolvePredefinedString(
        "zzReport", data, internalClock, lastReportFilename, APP_VERSION,
        contextName, contextTitle, lastReportName, lastPermissionName,
        lastQuestionAnswer);

    // Use \\b to prevent matching "zzReportFile"
    QRegularExpression nameRx("\\{[!$]zzReport\\}|\\$zzReport\\b",
                              QRegularExpression::CaseInsensitiveOption);
    result.replace(nameRx, val);
  }
  ...
  if (result.contains("zzPermission", Qt::CaseInsensitive)) {
    QString val = ScriptUtils::resolvePredefinedString(
        "zzPermission", data, internalClock, lastReportFilename, APP_VERSION,
        contextName, contextTitle, lastReportName, lastPermissionName,
        lastQuestionAnswer);

    // Use \\b to prevent partial matching
    QRegularExpression permRx("\\{[!$]zzPermission\\}|\\$zzPermission\\b",
                              QRegularExpression::CaseInsensitiveOption);
  ...
  if (result.contains("zzStatus", Qt::CaseInsensitive)) {
    QString val = ScriptUtils::resolvePredefinedString(
        "zzPermission", data, internalClock, lastReportFilename, APP_VERSION,
        contextName, contextTitle, lastReportName, lastPermissionName,
        currentStatus, lastQuestionAnswer);

    QRegularExpression statusRx("\\{[!$]zzStatus\\}|\\$zzStatus\\b",
                                QRegularExpression::CaseInsensitiveOption);
  ...
  if (result.contains("zzAnswer", Qt::CaseInsensitive)) {
    QString val = ScriptUtils::resolvePredefinedString(
        "zzAnswer", data, internalClock, lastReportFilename, APP_VERSION,
        contextName, contextTitle, lastReportName, lastPermissionName,
        currentStatus, lastQuestionAnswer);

    QRegularExpression ansRx("\\{[!$]zzAnswer\\}|\\$zzAnswer",
                             QRegularExpression::CaseInsensitiveOption);
  ...
  if (varIter.hasNext()) {
    ...
    while (varIter.hasNext()) {
      ...
      if (!startTime.isValid() && !contextName.isEmpty()) {
        ...
        QSettings settings(settingsFile, QSettings::IniFormat);
        QVariant val =
            settings.value("Assignments/" + contextName + "_start_time");
        if (val.isValid()) {
      ...
      if (prefix == "!") {
        ...
        QVariant val = getTimeVariableValue(varName, contextMinTime,
                                            contextMaxTime, contextStartTime,
                                            creationTime, deadline, nextRemind);

        if (val.isValid()) {
          if (format == "d") {
            if (val.userType() == QMetaType::QDateTime)
              valueStr = val.toDateTime().toString("yyyy-MM-dd");
            else if (val.userType() == QMetaType::QDate)
              valueStr = val.toDate().toString("yyyy-MM-dd");
            else
              valueStr = val.toString();
          } else if (format == "t") {
            if (val.userType() == QMetaType::QDateTime)
              valueStr = val.toDateTime().toString("HH:mm");
            else if (val.userType() == QMetaType::QTime)
              valueStr = val.toTime().toString("HH:mm");
            else
              valueStr = val.toString();
          } else if (format == "i") {
            int secs = 0;
            if (val.canConvert<int>())
              ...
              secs = QTime(0, 0).secsTo(val.toTime());
            valueStr = ScriptUtils::formatDurationString(secs);
          } else if (format == "dt") {
            if (val.userType() == QMetaType::QDateTime)
              valueStr = val.toDateTime().toString("yyyy-MM-dd HH:mm");
            else
              valueStr = val.toString();
          } else {
            if (val.userType() == QMetaType::QTime)
              valueStr = val.toTime().toString("HH:mm:ss");
            else if (val.userType() == QMetaType::QDateTime)
              valueStr = val.toDateTime().toString("yyyy-MM-dd HH:mm:ss");
...
void CyberDom::loadAndParseScript(const QString &filePath) {
  ...
  qDebug() << "   Title:   "
           << QString("%1's Virtual Master")
                  .arg(scriptParser->getScriptData().general.masterName);
  qDebug() << "==========================================";
...
void CyberDom::applyScriptSettings() {
  ...
  QString domType = scriptParser->getDomType();
  if (domType.isEmpty())
    domType = "Master";

  if (!data.general.title.isEmpty()) {
    setWindowTitle(data.general.title);
  } else if (data.general.title.isEmpty() && !subName.isEmpty()) {
    setWindowTitle(QString("%1's Virtual %2").arg(subName).arg(domType));
  } else {
    setWindowTitle(QString("CyberDom"));
  }
...
void CyberDom::setupInitialStatus() {
  ...
  if (scriptParser && scriptParser->getStatusSections().isEmpty()) {
    ...
    QMessageBox::critical(this, "Error",
                          "No status sections could be loaded from the script. "
    ...
    QMessageBox::warning(
        this, "Warning",
        "Using '" + savedStatus +
            "' as the default status, but functionality may be limited.");
  }
...
void CyberDom::changeStatus(const QString &statusName, bool forceAsSubStatus) {
  ...
  if (!statuses.contains(statusName.toLower())) {
    qDebug() << "[Status] ERROR: Status '" << statusName
             << "' not found in script data.";
    return;
  }
  ...
  if (isSubStatus) {
    ...
    if (currentStatuses.contains(currentStatus.toLower())) {
      if (!currentStatuses.value(currentStatus.toLower()).isSubStatus) {
        ...
        qDebug() << "[Status] Pushing" << currentStatus
                 << "to history. New sub-status is" << statusName;
      }
    }
  } else {
    // This is a Primary Status. Clear the History.
    if (!statusHistory.isEmpty()) {
      qDebug() << "[Status] Clearing status history. New primary status is"
               << statusName;
      statusHistory.clear();
    }
  }
  ...
  if (!statusHistory.isEmpty()) {
    ...
    if (scriptParser) {
      QString scriptDefault =
          scriptParser->getScriptData().general.defaultStatus;
...
void CyberDom::updateStatusDisplay() {
  // Format the status name for display (replace underscores with spaces,
  // capitalize words)
...
void CyberDom::updateSigninWidgetsVisibility(const StatusSection &status) {
  if (!status.signinIntervalMin.isEmpty()) {
    if (signinRemainingSecs == -1) {
      signinRemainingSecs = parseTimeRangeToSeconds(
          status.signinIntervalMin + "," + status.signinIntervalMax);
...
void CyberDom::updateAvailableActions() {
  if (!scriptParser) {
    qDebug()
        << "[ERROR] updateAvailableActions called, but scriptParser is null.";
    return;
  }
  ...
  if (statuses.contains(lowerStatus)) {
    ...
    qDebug() << "[WARNING] updateAvailableActions: Status '" << currentStatus
             << "' not found. Using default permissions.";
  }
...
int CyberDom::getMeritsFromIni() const { return ui->progressBar->value(); }
...
void CyberDom::initializeUiWithIniFile() {
  ...
  // This block handles setting initial merits/status from the script's [init]
  // section
  if (scriptParser) {
    const ScriptData &data = scriptParser->getScriptData();
    ...
    if (settings.contains("User/BeginTime")) {
      ...
      scriptBeginTime = QDateTime::fromString(
          settings.value("User/BeginTime").toString(), Qt::ISODate);
    } else {
      // First Run! Set to current internal time
      scriptBeginTime = internalClock;
      // We rely on the destructor to save this, matching the "User/Initialized"
      // logic
    }

    // Handle Last Close Time
    if (settings.contains("User/LastCloseTime")) {
      lastAppCloseTime = QDateTime::fromString(
          settings.value("User/LastCloseTime").toString(), Qt::ISODate);
    }
...
void CyberDom::updateMerits(int newMerits) {
  ...
  if (delta > 0)
    todayStats.meritsGained += delta;
  else if (delta < 0)
    todayStats.meritsLost += qAbs(delta);
...
QStringList CyberDom::getAvailableJobs() {
  ...
  if (!scriptParser) {
    qDebug() << "[ERROR] getAvailableJobs called, but scriptParser is not "
                "initialized.";
    return jobList;
  }
    ...
    if (!shouldRunToday && (job.intervalMin > 0 || job.firstIntervalMin > 0)) {
      ...
      if (!lastDoneDate.isValid()) {
        ...
        if (job.firstIntervalMin > 0) {
          ...
          QString firstDueKey =
              QString("JobCompletion/%1_firstDue").arg(job.name);
          QDate firstDueDate = QDate::fromString(
              settings.value(firstDueKey).toString(), Qt::ISODate);

          if (!firstDueDate.isValid()) {
            // This is the first time we're seeing this job. Set its initial due
            // date.
            int intervalDays;
            if (job.firstIntervalMax > job.firstIntervalMin) {
              intervalDays = QRandomGenerator::global()->bounded(
                  job.firstIntervalMin, job.firstIntervalMax + 1);
            } else {
        ...
        if (job.intervalMin > 0) {
          ...
          if (job.intervalMax > job.intervalMin) {
            intervalDays = QRandomGenerator::global()->bounded(
                job.intervalMin, job.intervalMax + 1);
          } else {
...
void CyberDom::assignJobFromTrigger(QString section) {
  ...
  QMap<QString, QStringList> sectionData =
      scriptParser->getRawSectionData(section);
...
void CyberDom::addJobToAssignments(QString jobName, bool isAutoAssign) {
  ...
  if (scriptParser &&
    ...
    const JobDefinition &def =
        scriptParser->getScriptData().jobs.value(jobName.toLower());

    if (def.isLineWriting &&
        def.lineSelectMode.compare("Random", Qt::CaseInsensitive) == 0) {
      if (!def.lines.isEmpty()) {
        ...
        QSettings settings(settingsFile, QSettings::IniFormat);
        settings.setValue("Assignments/" + jobName + "_selected_line",
                          selectedLine);
  ...
  if (scriptParser &&
    ...
    if (!deadlineSet) {
      ...
      deadlineSet = true;
      qDebug() << "[DEBUG] No Respite or EndTime found. Defaulting to 48-hour "
                  "deadline.";
    }
    ...
    QString remindRange;
    if (!jobDef.remindIntervalMin.isEmpty() ||
        !jobDef.remindIntervalMax.isEmpty()) {
      // Use Job's specific interval
      remindRange = jobDef.remindIntervalMin.isEmpty()
                        ? jobDef.remindIntervalMax
                        : jobDef.remindIntervalMin;
      if (!jobDef.remindIntervalMin.isEmpty() &&
          !jobDef.remindIntervalMax.isEmpty()) {
        remindRange = jobDef.remindIntervalMin + "," + jobDef.remindIntervalMax;
      }
    } else if (!scriptParser->getScriptData()
                    .general.globalRemindInterval.isEmpty()) {
  ...
  if (scriptParser &&
    ...
    if (isAutoAssign) {
      if (data.statuses.contains(currentStatus.toLower())) {
        const StatusDefinition &statusDef =
            data.statuses.value(currentStatus.toLower());
    ...
    // (If jobDef.announce is -1 AND general.announceJobs is false, showMessage
    // remains (false)
    ...
    if (showMessage) {
      if (isInterruptAllowed()) {
        ...
        QMessageBox::information(
            this, "New Assignment",
            "A new job has been added to your assignments: " + title);
      }
    }
  }
...
void CyberDom::checkPunishments() {
  ...
  for (const QString &name : activeAssignments) {
    ...
    if (isJob) {
      ...
      // const PunishmentDefinition &punDef is already available from
      // getPunishmentDefinition
    ...
    } else if (jobRemindIntervals.contains(name) &&
               now >= jobNextReminds.value(name)) {
      // Repeated Reminder
      if (isInterruptAllowed()) {
        QMessageBox::information(
            this, "Assignment Late",
            QString("You are late completing %1").arg(name));
      }
...
void CyberDom::addPunishmentToAssignments(const QString &punishmentName,
                                          int amount) {
  if (amount <= 0)
    return;
  if (!scriptParser)
  ...
  if (!punDef) {
    qDebug() << "[WARNING] Punishment section not found in ScriptData:"
             << punishmentName;
    return;
  }
  ...
  if (punDef->accumulative) {
    for (const QString &activeName : activeAssignments) {
      const PunishmentDefinition *activeDef =
          getPunishmentDefinition(activeName);
  ...
  if (targetInstance.isEmpty()) {
    ...
    // Save Creation Time
    QSettings settings(settingsFile, QSettings::IniFormat);
    // Only save if new (don't overwrite if we are just adding to an existing
    // accumulative punishment
    if (!settings.contains("Assignments/" + targetInstance +
                           "_creation_time")) {
      settings.setValue("Assignments/" + targetInstance + "_creation_time",
                        internalClock);
    }

    if (isNewInstance && punDef->isLineWriting &&
        punDef->lineSelectMode.compare("Random", Qt::CaseInsensitive) == 0) {
      if (!punDef->lines.isEmpty()) {
        int idx =
            ScriptUtils::randomInRange(0, punDef->lines.size() - 1, false);
        QString selectedLine = punDef->lines[idx];

        QSettings settings(settingsFile, QSettings::IniFormat);
        settings.setValue("Assignments/" + targetInstance + "_selected_line",
                          selectedLine);
        qDebug() << "[LineWriter] Select=Random picked:" << selectedLine;
      }
    }
  } else {
    qDebug() << "[DEBUG] Accumulating" << amount
             << "into existing:" << targetInstance;
  }
  ...
  if (isNewInstance) {
    if (scriptParser) {
      QString proc =
          scriptParser->getScriptData().eventHandlers.punishmentGiven;
      if (!proc.isEmpty())
        runProcedure(proc);
      if (!punDef->announceProcedure.isEmpty())
        runProcedure(punDef->announceProcedure);
    }
    ...
    if (!punDef->respite.isEmpty()) {
      ...
      QStringList respiteParts = respiteStr.split(":");
      if (respiteParts.size() >= 2) {
        deadline = deadline.addSecs(respiteParts[0].toInt() * 3600 +
                                    respiteParts[1].toInt() * 60);
        deadlineSet = true;
        qDebug() << "[DEBUG] Punishment deadline set from Respite:"
                 << deadline.toString("MM-dd-yyyy hh:mm AP");
      }
    }
    ...
    if (!deadlineSet && !punDef->valueUnit.isEmpty()) {
      ...
      if (deadlineSet) {
        qDebug() << "[DEBUG] Punishment deadline set from ValueUnit:"
                 << deadline.toString("MM-dd-yyyy hh:mm AP");
      }
    }
    ...
    if (!punDef->remindIntervalMin.isEmpty()) {
      ...
      if (!punDef->remindIntervalMax.isEmpty())
        remindRange += "," + punDef->remindIntervalMax;
    } else if (!scriptParser->getScriptData()
                    .general.globalRemindInterval.isEmpty()) {
      remindRange = scriptParser->getScriptData().general.globalRemindInterval;
    } else {
    ...
    if (remindRange.compare("Never", Qt::CaseInsensitive) == 0) {
      // Explicitly disabled
      qDebug() << "[DEBUG] RemindInterval is 'Never' for punishment:"
               << targetInstance;
    } else {
      int intervalSec = parseTimeRangeToSeconds(remindRange);
      if (intervalSec > 0)
        jobRemindIntervals[targetInstance] = intervalSec;
    }
...
void CyberDom::applyPunishment(int severity, const QString &group,
  ...
  if (severity < globalMin) {
    qDebug() << "[Punish] Severity" << severity << "increased to global minimum"
             << globalMin;
    severity = globalMin;
  }
  ...
  if (globalMax > 0 && severity > globalMax) {
    qDebug() << "[Punish] Severity" << severity << "capped at global maximum"
             << globalMax;
    severity = globalMax;
  }
  ...
  if (punishmentName.isEmpty()) {
    ...
    for (const PunishmentDefinition &def : allPunishments.values()) {
      ...
      if (!targetGroups.isEmpty()) {
        ...
        for (const QString &pg : def.groups) {
          if (targetGroups.contains(pg.trimmed().toLower())) {
            groupMatch = true;
            break;
          }
        }
    ...
    if (bestChunkSize > 0 && bestChunkSize < severity) {
      qDebug() << "[Punish] Splitting severity" << severity << "into chunk"
               << bestChunkSize << "and remainder"
               << (severity - bestChunkSize);
      applyPunishment(bestChunkSize, group, message);
      applyPunishment(severity - bestChunkSize, group, message);
      return;
    } else {
      qDebug() << "[Punish] Unable to match or split severity" << severity
               << "in group" << group;
      QMessageBox::warning(this, "Error",
                           "No suitable punishment (or combination of "
                           "punishments) could be found for this severity.");
      return;
    }
  }
  ...
  while (true) {
    ...
    QMap<QString, QStringList> rawData =
        scriptParser->getRawSectionData("punishment-" + punishmentName);
    ...
    if (!cleanTitle.isEmpty())
      cleanTitle[0] = cleanTitle[0].toUpper();

    if (!message.isEmpty()) {
      // Custom message from script
      displayMsg =
          replaceVariables(message, punDef.name, punDef.title, severity);
      ...
      if (titleHadPlaceholder) {
        ...
        // Standard fallback: "You have been punished with 5 Spanks of
        // Spanking."
    ...
    QPushButton *declineBtn =
        msgBox.addButton("Decline", QMessageBox::RejectRole);
...
void CyberDom::executePunish(const QString &severityValue,
                             const QString &message, const QString &group) {
  if (!scriptParser)
    return;
...
QString CyberDom::selectPunishmentFromGroup(int severity,
                                            const QString &group) {
  ...
  for (const PunishmentDefinition &punDef : allPunishments.values()) {
    ...
    if (suitable) {
      ...
      if (minWeight == maxWeight) {
        ...
        finalWeight =
            QRandomGenerator::global()->bounded(minWeight, maxWeight + 1);
      }
  ...
  if (!weightedEligiblePunishments.isEmpty()) {
    int index =
        QRandomGenerator::global()->bounded(weightedEligiblePunishments.size());
...
void CyberDom::checkFlagExpiry() {
  ...
  for (auto it = flags.begin(); it != flags.end(); ++it) {
    if (it.value().expiryTime.isValid() &&
        internalClock >= it.value().expiryTime) {
  ...
  for (const QString &flagName : expiredFlags) {
    ...
    if (scriptParser) {
      ...
      const FlagDefinition &def =
          scriptParser->getScriptData().flagDefinitions.value(
              flagName.toLower());
...
bool CyberDom::startAssignment(const QString &assignmentName, bool isPunishment,
                               const QString &newStatus) {
  if (hasActiveBlockingPunishment()) {
    QMessageBox::warning(
        this, tr("Punishment Running"),
  ...
  if (!scriptParser) {
    ...
    if (isPunishment) {
      if (data.punishments.contains(lowerName)) {
        ...
        qDebug() << "[WARNING] Section not found for punishment: "
                 << assignmentName;
      }
    } else {
  ...
  if (!beforeProcedure.isEmpty()) {
    ...
    if (isFlagSet("zzDeny")) {
      qDebug() << "[StartAssignment] Start of" << assignmentName
               << "was denied by BeforeProcedure.";
      removeFlag("zzDeny");
      return false;
    }
  }
  ...
  QString startFlagName =
      (isPunishment ? "punishment_" : "job_") + assignmentName + "_started";
  ...
  if (scriptParser) {
    ...
    QString proc =
        isPunishment ? handlers.punishmentGiven : handlers.jobAnnounced;
    if (!proc.isEmpty())
      runProcedure(proc);
  }
  ...
  if (!newStatus.isEmpty()) {
    QString statusFlagName = (isPunishment ? "punishment_" : "job_") +
                             assignmentName + "_prev_status";
    QSettings settings(settingsFile, QSettings::IniFormat);
  ...
  if (!pointCameraText.isEmpty()) {
    ...
    QString resolvedPointText =
        replaceVariables(pointCameraText, assignmentName, assignmentTitle);
    triggerPointCamera(pointCameraText, cameraIntervalMin, cameraIntervalMax,
                       sourcePrefix + assignmentName);
  }
...
bool CyberDom::markAssignmentDone(const QString &assignmentName,
  ...
  if (isPunishment) {
    ...
    quickPenalty1 =
        (dur.quickPenalty1 != 0) ? QString::number(dur.quickPenalty1) : "";
    quickPenalty2 =
        (dur.quickPenalty2 != 0.0) ? QString::number(dur.quickPenalty2) : "";
    ...
    bool isTimeBased =
        (valueUnit == "day" || valueUnit == "hour" || valueUnit == "minute");
    ...
    if (!data.jobs.contains(lowerName)) {
      qDebug() << "[WARNING] Section not found for completed assignment: job-"
               << assignmentName;
      return false;
    }
    const JobDefinition &def = data.jobs.value(lowerName);
  ...
  if (!beforeDoneProcedure.isEmpty()) {
    ...
    // Check if the procedure set the zzDeny flag
    if (isFlagSet("zzDeny")) {
      qDebug() << "[markAssignmentDone] 'Done' for" << assignmentName
               << "was denied by BeforeDoneProcedure.";
    }
  }
  ...
  QDateTime start =
      settings.value("Assignments/" + assignmentName + "_start_time")
          .toDateTime();
  int elapsed =
      start.isValid() ? static_cast<int>(start.secsTo(internalClock)) : 0;

  // Check Timed Punishment
  if (isPunishment &&
      (valueUnit == "minute" || valueUnit == "hour" || valueUnit == "day")) {
    int requiredSecs = 0;
    ...
    if (requiredSecs > 0 && elapsed < requiredSecs) {
      ...
      QMessageBox::warning(this, "Too Quick",
                           QString("This punishment is not complete. You must "
                                   "continue for another %1.")
  ...
  if (!isPunishment && !minTime.isEmpty()) {
    ...
    if (required > 0 && elapsed < required) {
      lastEarlyMinutes =
          static_cast<int>(std::ceil((required - elapsed) / 60.0));
      ...
      bool showMsg = !quickMessage.isEmpty() || !quickPenalty1.isEmpty() ||
                     !quickPenalty2.isEmpty();
      if (showMsg) {
        ...
        QMessageBox::information(
            this, "Too Quick",
            replaceVariables(msg, assignmentName, "", 0, minTime));
      }
  ...
  if (!maxTime.isEmpty()) {
    ...
    if (limit > 0 && elapsed > limit) {
      ...
      qDebug() << "[Assignment] User was slow by" << lastLatenessMinutes
               << "minutes.";
      ...
      if (!dur.slowMessage.isEmpty()) {
        QMessageBox::information(this, "Too Slow",
                                 replaceVariables(dur.slowMessage,
                                                  assignmentName, "", 0,
                                                  minTime, dur.maxTimeStart));
      }
  ...
  if (mustStart) {
    ...
    if (!isFlagSet(startFlagName)) {
      QMessageBox::warning(
          this, "Not Started",
          QString("This ") + (isPunishment ? "punishment" : "job") +
              " must be started before it can be marked done.");
      return false;
    }
  }
  ...
  QString statusFlagName =
      (isPunishment ? "punishment_" : "job_") + assignmentName + "_prev_status";
  QString savedPrevStatus =
      settings.value("Assignments/" + statusFlagName, "").toString();
  ...
  if (!addMerit.isEmpty()) {
    ...
    if (meritPoints != 0) {
      ...
      if (currentMerits > maxMerits)
        currentMerits = maxMerits;
      if (currentMerits < minMerits)
        currentMerits = minMerits;
  ...
  if (!isPunishment) {
    todayStats.punishmentsCompleted.append(
        assignmentName + " (" + QTime::currentTime().toString("hh:mm AP") +
        ")");
    QString settingsKey =
        QString("JobCompletion/%1_lastDone").arg(assignmentName);
    settings.setValue(settingsKey, internalClock.date().toString(Qt::ISODate));
    qDebug() << "[Scheduler] Job" << assignmentName
             << "completed. Saving lastDone date:"
             << internalClock.date().toString(Qt::ISODate);
  } else {
    todayStats.jobsCompleted.append(assignmentName + " (" +
    ...
    if (jobDef.oneTime) {
      QString oneTimeKey =
          QString("JobCompletion/$1_oneTimeDone").arg(assignmentName);
      settings.setValue(oneTimeKey, true);
      qDebug() << "[Scheduler] OneTime job" << assignmentName
               << "completed. Flagging as done forever.";
    }
  }
...
bool CyberDom::checkInterruptableAssignments() {
  ...
  for (const QString &name : active) {
    ...
    QString startFlagName =
        (isPun ? "punishment_" : "job_") + name + "_started";
    if (!isFlagSet(startFlagName)) {
      continue;
    }
    ...
    if (!isInterruptable) {
      ...
      qDebug() << "[Interrupt] Non-interruptable assignment" << name
               << "was active. Aborting.";
      QMessageBox::warning(this, "Assignment Aborted",
                           QString("Your non-interruptable assignment '%1' was "
                                   "aborted because you used the application.")
                               .arg(name));
...
void CyberDom::deleteAssignment(const QString &assignmentName,
                                bool isPunishment) {
  ...
  if (scriptParser) {
    ...
    if (isPunishment) {
      if (data.punishments.contains(lowerName)) {
        ...
        qDebug()
            << "[WARNING] Section not found for deleted assignment: punishment-"
            << assignmentName;
      }
    } else {
      if (data.jobs.contains(lowerName)) {
        ...
        qDebug() << "[WARNING] Section not found for deleted assignment: job-"
                 << assignmentName;
      }
    }
  }
...
QString CyberDom::getClothingReportPrompt() const {
  ...
  if (scriptParser) {
    QMap<QString, QStringList> rawStatusData =
        scriptParser->getRawSectionData("status-" + currentStatus);

    // Find the "ClothReport" key (case-insensitive)
    for (auto it = rawStatusData.constBegin(); it != rawStatusData.constEnd();
         ++it) {
...
void CyberDom::processClothingReport(const QList<ClothingItem> &wearingItems,
                                     bool isNaked) {
  QStringList errors;
  ...
  qDebug() << "[Clothing Check] User Wearing:" << wearingItems.size()
           << "items.";
  ...
  for (const QString &forbidden : forbiddenClothingChecks) {
    for (const ClothingItem &item : wearingItems) {
      if (itemMatchesCheck(item, forbidden)) {
        qDebug() << "[Clothing Check] FAILED: Wearing forbidden item -"
                 << forbidden;
        hasFailures = true;
  ...
  if (isNaked) {
    ...
    for (auto it = itemsByType.constBegin(); it != itemsByType.constEnd();
         ++it) {
  ...
  QString summary = isNaked ? QString("Naked (%1)").arg(time)
                            : QString("Dressed (%1)").arg(time);
  ...
  QMessageBox::information(this, "Report Submitted",
                           "Your clothing report has been submitted.");
...
void CyberDom::storeClothingReport(const QString &reportText) {
  ...
  QString reportKey =
      QString("ClothingReport_%1").arg(timestamp.toString("yyyyMMdd_hhmmss"));
  ...
  settings.setValue("LatestClothingReportTime",
...
QMap<QDate, QStringList> CyberDom::getHolidays() const {
  ...
  for (const QString &sectionName : allSectionNames) {
    ...
    QMap<QString, QStringList> rawData =
        scriptParser->getRawSectionData(sectionName);
...
void CyberDom::executeQuestion(const QString &questionKey,
                               const QString &title) {
  ...
  lastResponseDuration =
      start.secsTo(QDateTime::currentDateTime()); // End Timer
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    for (const auto &answerBlock : questionData.answers) {
      ...
      bool textMatch = (answerBlock.answerText.compare(
                            selectedAnswer, Qt::CaseInsensitive) == 0);

      // Check: Does it match the Procedure Name
      bool procMatch = (answerBlock.procedureName.compare(
                            selectedAnswer, Qt::CaseInsensitive) == 0);

      // Check: Does it match the explicit variable value
      bool varMatch = (!answerBlock.variableValue.isEmpty() &&
                       answerBlock.variableValue.compare(
                           selectedAnswer, Qt::CaseInsensitive) == 0);
      ...
      if (textMatch || procMatch || varMatch) {
        ...
        if (answerBlock.punishMin != 0 || answerBlock.punishMax != 0) {
          ...
          int maxSeverity =
              answerBlock.punishMax ? answerBlock.punishMax : minSeverity;
          ...
          severity =
              QRandomGenerator::global()->bounded(minSeverity, maxSeverity + 1);
#else
          ...
          severity =
              minSeverity + std::rand() % (maxSeverity - minSeverity + 1);
#endif
...
void CyberDom::performCounterOperation(const QString &opString,
  ...
  qDebug() << "[DEBUG] Counter Op:" << varName << opType << valToApply << "="
           << result;
}
...
void CyberDom::modifyMerits(const QString &value, const QString &opType) {
  ...
  updateMerits(currentMerits);
  qDebug() << "[Merits]" << opType << "points:" << points
           << "New Total:" << currentMerits;
}
...
bool CyberDom::runProcedure(const QString &procedureName) {
  ...
  if (!proc.preStatuses.isEmpty()) {
    ...
    if (!statusMatch) {
      qDebug() << "[Procedure Skipped]" << proc.name
               << "is not allowed in status:" << currentStatus;
      return false; // Signal that we did not run
    }
  }

  // --- 2. Check Time Restrictions ---
  if (!isTimeAllowed(proc.notBeforeTimes, proc.notAfterTimes,
                     proc.notBetweenTimes)) {
    qDebug() << "[DEBUG] Procedure skipped due to time restrictions:"
             << proc.name;
    return false; // Signal that we did not run
  }
  ...
  if (proc.selectMode == ProcedureSelectMode::Random) {
    ...
    for (const ScriptAction &a : proc.actions) {
      if (a.type == ScriptActionType::ProcedureCall)
        procCallCount++;
    }
    if (procCallCount > 0) {
      targetRandomIndex =
          ScriptUtils::randomInRange(0, procCallCount - 1, false);
    }
  }
  ...
  for (const ScriptAction &action : proc.actions) {
    ...
    case ScriptActionType::Timer: {
      ...
      if (scriptParser &&
          scriptParser->getScriptData().timers.contains(timerName)) {
        const TimerDefinition &def =
            scriptParser->getScriptData().timers.value(timerName);

        // This logic is copied from applyScriptSettings
        QString startRange = def.startTimeMin;
        if (!def.startTimeMax.isEmpty() &&
            def.startTimeMax != def.startTimeMin) {
          startRange += "," + def.startTimeMax;
        }
        int startSecs = parseTimeRangeToSeconds(startRange);
        QTime startTime = QTime(0, 0).addSecs(startSecs);
        ...
        qDebug() << "[WARN] Procedure tried to activate unknown timer:"
                 << timerName;
      }
      break;
    }
    ...
    case ScriptActionType::PointCamera: {
      ...
      triggerPointCamera(action.value, minInt, maxInt,
                         "Procedure_" + proc.name);
      break;
    }
...
bool CyberDom::loadSessionData(const QString &path) {
  ...
  lastClothingInstructions =
      session.value("Session/LastClothingInstructions").toString();
  ...
  lastPunishmentSeverity =
      session.value("Session/LastPunishmentSeverity", 0).toInt();
  ...
  QDateTime lastInternal = QDateTime::fromString(
      session.value("Session/InternalClock").toString(), Qt::ISODate);
  QDateTime lastSystem = QDateTime::fromString(
      session.value("Session/LastSystemTime").toString(), Qt::ISODate);

  // --- 1. Load Status History ---
  QStringList historyList =
      session.value("Session/StatusHistory").toStringList();
  statusHistory.clear();
  for (const QString &s : historyList)
  ...
  for (const QString &key : session.childKeys()) {
    ...
    jobDeadlines[cleanKey] =
        QDateTime::fromString(session.value(key).toString(), Qt::ISODate);
  }
  session.endGroup();
  ...
  QStringList expiredList =
      session.value("ExpiredAssignmentsSet").toStringList();
  ...
  for (const QString &fname : flagGroups) {
    ...
    fd.name = fname;
    fd.setTime =
        QDateTime::fromString(session.value("SetTime").toString(), Qt::ISODate);
    fd.expiryTime = QDateTime::fromString(
        session.value("ExpiryTime").toString(), Qt::ISODate);
  ...
  for (const QString &key : session.childKeys()) {
    QDateTime dt =
        QDateTime::fromString(session.value(key).toString(), Qt::ISODate);
    if (dt.isValid())
      permissionNextAvailable[key] = dt;
  }
  session.endGroup();
  ...
  for (const QString &key : session.childKeys()) {
    QDateTime dt =
        QDateTime::fromString(session.value(key).toString(), Qt::ISODate);
    if (dt.isValid())
      permissionFirstDenied[key] = dt;
  }
  session.endGroup();

  QStringList pendingNotes =
      session.value("Session/PermissionNotifications").toStringList();
  permissionNotificationsPending =
      QSet<QString>(pendingNotes.begin(), pendingNotes.end());
...
void CyberDom::saveSessionData(const QString &path) const {
  ...
  session.setValue("Session/LastClothingInstructions",
                   lastClothingInstructions);
  session.setValue("Session/LastJobsRun",
                   lastScheduledJobsRun.toString(Qt::ISODate));
  session.setValue("Session/LastDayMeritsGiven",
                   lastDayMeritsGiven.toString(Qt::ISODate));
  ...
  session.beginGroup("Expiry");
  for (auto it = jobExpiryTimes.constBegin(); it != jobExpiryTimes.constEnd();
       ++it) {
    session.setValue(it.key(), it.value().toString(Qt::ISODate));
  }
  session.endGroup();
  ...
  session.setValue("ExpiredAssignmentsSet",
                   QStringList(expiredAssignments.values()));

  session.beginGroup("RemindIntervals");
  for (auto it = jobRemindIntervals.constBegin();
       it != jobRemindIntervals.constEnd(); ++it) {
    session.setValue(it.key(), it.value());
  }
  session.endGroup();

  session.beginGroup("NextReminds");
  for (auto it = jobNextReminds.constBegin(); it != jobNextReminds.constEnd();
       ++it) {
    session.setValue(it.key(), it.value().toString(Qt::ISODate));
  }
  session.endGroup();
  ...
  session.beginGroup("LateMerits");
  for (auto it = jobLateMerits.constBegin(); it != jobLateMerits.constEnd();
       ++it) {
    session.setValue(it.key(), it.value());
  }
  session.endGroup();
  ...
  session.beginGroup("Amounts");
  for (auto it = punishmentAmounts.constBegin();
       it != punishmentAmounts.constEnd(); ++it) {
    session.setValue(it.key(), it.value());
  }
  session.endGroup();
  session.endGroup(); // Assignments
  ...
  // --- REMOVED: Answers Group (Saved in user_settings.ini via
  // saveQuestionAnswers) ---

  // --- 5. Save Counters ---
  session.beginGroup("Counters");
  session.beginGroup("Reports");
  for (auto it = reportCounts.constBegin(); it != reportCounts.constEnd();
       ++it) {
    session.setValue(it.key(), it.value());
  }
  session.endGroup();
  session.beginGroup("Confessions");
  for (auto it = confessionCounts.constBegin();
       it != confessionCounts.constEnd(); ++it) {
    session.setValue(it.key(), it.value());
  }
  session.endGroup();
  session.beginGroup("Permissions");
  for (auto it = permissionCounts.constBegin();
       it != permissionCounts.constEnd(); ++it) {
    session.setValue(it.key(), it.value());
  }
  session.endGroup();

  session.beginGroup("PermissionTimers");
  for (auto it = permissionNextAvailable.constBegin();
       it != permissionNextAvailable.constEnd(); ++it) {
    session.setValue(it.key(), it.value().toString(Qt::ISODate));
  }
  session.endGroup();

  session.beginGroup("PermissionDeniedStats");
  for (auto it = permissionFirstDenied.constBegin();
       it != permissionFirstDenied.constEnd(); ++it) {
    session.setValue(it.key(), it.value().toString(Qt::ISODate));
  }
  session.endGroup();

  session.setValue("Session/PermissionNotifications",
                   QStringList(permissionNotificationsPending.values()));
  session.endGroup(); // Counters
...
void CyberDom::onResetSigninTimer() {
  ...
  signinRemainingSecs = parseTimeRangeToSeconds(status.signinIntervalMin + "," +
                                                status.signinIntervalMax);
  ui->timerLabel->setStyleSheet("");
  signinTimer->start(1000);
...
bool CyberDom::isTimeAllowed(
    const QStringList &notBefore, const QStringList &notAfter,
...
bool CyberDom::isInterruptAllowed() const {
  if (!scriptParser)
    return true; // Failsafe, allow popups

  const auto &interruptStatuses =
      scriptParser->getScriptData().general.interruptStatuses;

  // If the list is empty, it means no restrictions are set, so popups are
  // allowed
...
QStringList CyberDom::getAssignmentResources(const QString &name,
  ...
  if (isPunishment) {
    for (auto it = data.punishments.constBegin();
         it != data.punishments.constEnd(); ++it) {
...
bool CyberDom::isAssignmentLongRunning(const QString &name,
                                       bool isPunishment) const {
  if (!scriptParser)
    return false;
...
QList<CalendarEvent> CyberDom::getCalendarEvents() {
  ...
  if (scriptParser) {
    QMap<QString, QStringList> holSec =
        scriptParser->getRawSectionData(QStringLiteral("holidays"));
...
QSet<QString> CyberDom::getResourcesInUse() const {
  ...
  for (const QString &name : activeAssignments) {
    ...
    bool isPun =
        scriptParser->getScriptData().punishments.contains(name.toLower());
...
bool CyberDom::hasActiveBlockingPunishment() const {
  ...
  for (const QString &name : activeAssignments) {
    ...
    if (punDef->longRunning)
      continue;
    if (punDef->valueUnit.compare("day", Qt::CaseInsensitive) == 0)
      continue;
    ...
    if (data.punishments.contains(lowerName)) {
      ...
      if (vu == "minute" || vu == "hour" || vu == "day") {
        ...
        if (vu == "minute")
          ...
          totalSecs = amount * 3600;
        else if (vu == "day")
          totalSecs = amount * 86400;
...
const PunishmentDefinition *
CyberDom::getPunishmentDefinition(const QString &instanceName) const {
  if (!scriptParser)
    return nullptr;
...
QString CyberDom::resolveInstruction(const QString &name,
                                     QStringList *chosenItems,
                                     QSet<QString> visited,
                                     bool isClothingContext) {
  if (!scriptParser)
    return QString();
  ...
  if (visited.contains(lowerName)) {
    qWarning() << "[Instructions] Infinite loop detected! Set" << name
               << "calls itself.";
  ...
  if (useCache) {
    ...
    if (def.changeMode == InstructionChangeMode::Program) {
      if (settings.contains(cacheKeyBase + "_text"))
        cacheValid = true;
    } else if (def.changeMode == InstructionChangeMode::Daily) {
      QString savedDate = settings.value(cacheKeyBase + "_date").toString();
      if (savedDate == internalClock.date().toString(Qt::ISODate))
        cacheValid = true;
    }

    if (cacheValid) {
      QString cachedText = settings.value(cacheKeyBase + "_text").toString();
      QStringList cachedChecks =
          settings.value(cacheKeyBase + "_checks").toStringList();
      QStringList cachedForbids =
          settings.value(cacheKeyBase + "_forbids").toStringList();
      QStringList cachedChosen =
          settings.value(cacheKeyBase + "_chosen").toStringList();
  ...
  QStringList *targetChosenItems =
      chosenItems ? chosenItems : &locallyChosenItems;

  for (const InstructionSet &set : def.sets) {
    if (!checkRequirements(set.ifFlagGroups, set.notIfFlagGroups))
      continue;
    if (!ScriptUtils::checkHistoryConditions(set.ifChosen, set.ifNotChosen,
                                             *targetChosenItems))
      continue;
    if (set.steps.isEmpty())
      continue;

    ScriptUtils::SelectMode mode = ScriptUtils::SelectMode::All;
    if (set.selectMode == InstructionSelectMode::First)
      mode = ScriptUtils::SelectMode::First;
    else if (set.selectMode == InstructionSelectMode::Random)
      mode = ScriptUtils::SelectMode::Random;

    ScriptUtils::processSelector(
        set.steps.size(), mode, [&](int index) -> bool {
          if (index < 0 || index >= set.steps.size())
            return false;
          const InstructionStep &step = set.steps[index];

          if (step.type == InstructionStepType::SetReference) {
            QString refText =
                resolveInstruction(step.setReference, targetChosenItems,
                                   visited, currentContextIsClothing);
            if (!refText.isEmpty()) {
  ...
  if (useCache) {
    ...
    settings.setValue(cacheKeyBase + "_date",
                      internalClock.date().toString(Qt::ISODate));
...
bool CyberDom::itemMatchesCheck(const ClothingItem &item,
                                const QString &checkName) const {
  if (!scriptParser)
    return false;
  ...
  if (!clothTypes.contains(typeName))
    return false;
...
void CyberDom::clearCurrentClothing() {
  ...
  QSettings settings(dir.absoluteFilePath("clothing.ini"),
                     QSettings::IniFormat);
...
int CyberDom::calculateSecondsFromTimeRange(const TimeRange &tr) {
  ...
  auto resolve = [&](const QString &s) -> int {
    ...
    if (val.startsWith("!") || val.startsWith("#")) {
      if (scriptParser)
        val = scriptParser->getVariable(val.mid(1));
    }
  ...
  return ScriptUtils::randomInRange(std::min(minSec, maxSec),
                                    std::max(minSec, maxSec), false);
}

bool CyberDom::checkRequirements(const QList<QStringList> &ifGroups,
                                 const QList<QStringList> &notIfGroups) {
  if (!scriptParser)
    return false;

  return ScriptUtils::checkConditions(
      ifGroups, notIfGroups, [this](const QString &n) { return isFlagSet(n); },
      [this](const QString &n) { return scriptParser->getVariable(n); });
}

void CyberDom::triggerPointCamera(const QString &text,
                                  const QString &minInterval,
                                  const QString &maxInterval,
  ...
  if (!minInterval.isEmpty()) {
    ...
    connect(timer, &QTimer::timeout, this,
            [this, sourceName]() { handleCameraTimer(sourceName); });

    QString range = minInterval;
    if (!maxInterval.isEmpty())
      range += "," + maxInterval;
    ...
    if (secs > 0) {
      ...
      qDebug() << "[Camera] Started interval for" << sourceName
               << "Next shot in" << secs << "s";
    }
  }
}
...
void CyberDom::trackPermissionEvent(const QString &name,
                                    const QString &result) {
  QString time = QTime::currentTime().toString("hh:mm AP");
  todayStats.permissionsAsked.append(
      QString("%1: %2 (%3)").arg(name, result, time));
}
...
void CyberDom::generateDailyReportFile(bool isEndOfDay) {
  ...
  QString filename =
      QString("Report_%1.html").arg(internalClock.toString("yyyy-MM-dd"));
...
void CyberDom::onMakeReportFile() {
  ...
  QString path = getReportsDirectory() + "/Report_" +
                 internalClock.toString("yyyy-MM-dd") + ".html";
...
void CyberDom::onViewReportFile() {
  ...
  if (!QFile::exists(path)) {
    ...
    generateDailyReportFile(false);
    if (!QFile::exists(path)) {
      QMessageBox::warning(this, "No Report",
                           "Could not find or generate a report for today.");
      return;
    }
  }
  ...
  // which causes most browsers to crash or fail to read files due to library
  // conflicts. We explicitly run 'xdg-open' with the LD_LIBRARY_PATH removed to
  // fix this.
  ...
  if (!process.startDetached()) {
    qWarning() << "[Report] Failed to launch xdg-open. Falling back to "
                  "QDesktopServices.";
...
QString CyberDom::getAssignmentDisplayName(const QString &assignmentName,
                                           bool isPunishment) const {
  if (!scriptParser)
  ...
  if (isPunishment) {
    ...
    if (def.isLineWriting &&
      ...
      QSettings settings(settingsFile, QSettings::IniFormat);
      QString line =
          settings.value("Assignments/" + assignmentName + "_selected_line")
              .toString();
      if (!line.isEmpty()) {
        int amt = getPunishmentAmount(assignmentName);
...
void CyberDom::executeStringAction(ScriptActionType type,
                                   const QString &value) {
  // Handle Drop$
  ...
  if (type == ScriptActionType::SetString) {
    ...
    QString text = QInputDialog::getText(
        this, "Input Required", resolvedContent, QLineEdit::Normal, "", &ok);
    lastResponseDuration =
        start.secsTo(QDateTime::currentDateTime()); // End Timer
    if (ok) {
    ...
    QDateTime start = QDateTime::currentDateTime(); // Start Timer
    QString text =
        QInputDialog::getText(this, "Input Required", resolvedContent,
    ...
    QString text = QInputDialog::getMultiLineText(
        this, "Input Required", resolvedContent, currentValue, &ok);
    lastResponseDuration =
        start.secsTo(QDateTime::currentDateTime()); // End Timer
    if (ok) {
      scriptParser->setVariable(varName, text);
...
void CyberDom::executeCounterAction(ScriptActionType type,
                                    const QString &value) {
  if (!scriptParser)
    return;
  ...
  if (type == ScriptActionType::InputCounter) {
    ...
    QDateTime start = QDateTime::currentDateTime(); // Start Timer
    int val =
        QInputDialog::getInt(this, "Input Required", replaceVariables(param), 0,
                             0, 2147483647, 1, &ok);
    lastResponseDuration =
        start.secsTo(QDateTime::currentDateTime()); // End Timer
    if (ok) {
  ...
  if (type == ScriptActionType::ChangeCounter) {
    ...
    QDateTime start = QDateTime::currentDateTime(); // Start Timer
    int val =
        QInputDialog::getInt(this, "Input Required", replaceVariables(param),
  ...
  int valToApply = ScriptUtils::resolveInt(
      param, [this](const QString &n) { return scriptParser->getVariable(n); });
  ...
  if (type != ScriptActionType::SetCounterVar) {
    ...
    currentVal =
        ScriptUtils::resolveInt("#" + varName, [this](const QString &n) {
...
void CyberDom::executeRandomCounterAction(const QString &value) {
  ...
  auto getVar = [this](const QString &n) {
    return scriptParser->getVariable(n);
  };
  ...
  // Save
  scriptParser->setVariable(varName, QString::number(result));
  qDebug() << "[Random#] Set" << varName << "to" << result << "(Range:" << min
           << "-" << max << ")";
}

QString CyberDom::getVariableValue(const QString &name,
                                   int contextSeverity) const {
  if (!scriptParser)
    return "0";
  ...
  if (name.compare("zzMerits", Qt::CaseInsensitive) == 0 ||
    ...
    int currentMerits = getMeritsFromIni();
    int startMerits =
        currentMerits - todayStats.meritsGained + todayStats.meritsLost;
    ...
    for (const QString &assignment : activeAssignments) {
      ...
      // Check if Started
      QString flagName =
          (isPun ? "punishment_" : "job_") + assignment + "_started";
    ...
    return QString::number(ScriptUtils::resolvePredefinedCounter(
        name, currentMerits, startMerits, maxMerits, minMerits, yellowMerits,
        redMerits, activeCount, jobCount, punishmentCount, dueTodayCount,
        jobsDueTodayCount, punishmentsDueTodayCount, unstartedCount,
        unstartedJobCount, unstartedPunishCount, startedCount, startedJobCount,
        startedPunishCount, overdueCount, overdueJobCount, overduePunishCount,
        currentDay, currentMonth, currentYear, isLeapYear, currentHour,
        currentMinute, currentSecond, secondsPassedToday, currentDayOfWeek,
        clothFaultsCount, lastPunishmentSeverity, lastLatenessMinutes,
        lastEarlyMinutes, contextSeverity));
  }
...
void CyberDom::executeTimeAction(ScriptActionType type, const QString &value) {
  ...
  if (type == ScriptActionType::InputDate ||
    ...
    if (type == ScriptActionType::InputDateDef) {
      ...
      if (secondComma != -1) {
        ...
        if (initStr.startsWith("!")) {
          ...
          QDateTime dt =
              ScriptUtils::parseDateString(replaceVariables(initStr));
          if (dt.isValid())
            defaultDate = dt.date();
        }
      } else {
    ...
    if (dlg.exec() == QDialog::Accepted) {
      ...
      qDebug() << "[InputDate] Set" << varName << "to"
  ...
  if (type == ScriptActionType::InputTime ||
    ...
    if (type == ScriptActionType::InputTimeDef) {
      ...
      if (secondComma != -1) {
        ...
        if (initStr.startsWith("!")) {
          ...
          if (!t.isValid())
            t = QTime::fromString(initStr, "HH:mm");
          if (t.isValid())
            defaultTime = t;
        }
      } else {
    ...
    if (dlg.exec() == QDialog::Accepted) {
      ...
      qDebug() << "[InputTime] Set" << varName << "to"
               << timeEdit->time().toString();
    }
    lastResponseDuration = start.secsTo(QDateTime::currentDateTime());
    return;
  }
  ...
  if (type == ScriptActionType::InputInterval ||
    ...
    if (dlg.exec() == QDialog::Accepted) {
      scriptParser->setTimeVariable(varName, timeEdit->time());
      qDebug() << "[InputInterval] Set" << varName << "to"
               << timeEdit->time().toString("HH:mm:ss");
    }
    lastResponseDuration = start.secsTo(QDateTime::currentDateTime());
    return;
  }
  ...
  if (type == ScriptActionType::AddDaysTime ||
      type == ScriptActionType::SubtractDaysTime) {
    ...
    if (currentVal.userType() == QMetaType::QDateTime) {
      ...
      qDebug() << "[AddDays] Adjusted date" << varName << "by" << days
               << "days";
    } else if (currentVal.canConvert<int>()) {
      int currentSecs = currentVal.toInt();
      int adjustSecs = days * 86400;
      newVal = currentSecs + adjustSecs;
      qDebug() << "[AddDays] Adjusted duration" << varName << "by" << days
               << "days";
    } else {
      newVal = days * 86400;
    }
  ...
  if (type == ScriptActionType::RoundTime) {
    ...
    if (currentVal.userType() == QMetaType::QDateTime) {
      ...
      if (remainder >= (baseSeconds / 2))
        epoch += (baseSeconds - remainder);
      else
        epoch -= remainder;
    ...
    scriptParser->setTimeVariable(varName, newVal);
    qDebug() << "[Round] Rounded" << varName << "to nearest" << baseSeconds
             << "s";
    return;
  }
    ...
    auto resolveSeconds = [&](const QString &s) -> int {
      if (s.startsWith("!")) {
        ...
        if (v.userType() == QMetaType::QTime)
          return QTime(0, 0, 0).secsTo(v.toTime());
...
void CyberDom::executeTimeExtraction(ScriptActionType type,
  ...
  if (counterName.startsWith("#"))
    counterName = counterName.mid(1);

  QString timeVarName = parts[1].trimmed();
  if (timeVarName.startsWith("!"))
    timeVarName = timeVarName.mid(1);
  ...
  if (type == ScriptActionType::ExtractDays) {
    if (val.canConvert<int>() && val.userType() != QMetaType::QDateTime &&
        val.userType() != QMetaType::QTime) {
...
void CyberDom::executeCounterToTime(ScriptActionType type,
                                    const QString &value) {
  if (!scriptParser)
    return;
  ...
  int val = ScriptUtils::resolveInt(
      counterParam, [this](const QString &n) { return getVariableValue(n); });
  ...
  qDebug() << "[Convert!] Set" << timeVarName << "to" << resultSeconds
           << "seconds (from" << val << "units)";
}

QVariant CyberDom::getTimeVariableValue(
    const QString &name, const QString &contextMinTime,
    const QString &contextMaxTime, const QDateTime &contextStartTime,
    const QDateTime &contextCreationTime, const QDateTime &contextDeadline,
    const QDateTime &contextNextRemind,
    const QDateTime &contextNextAllow) const {
  // Check Predefined Time Variables
  QVariant predefined = ScriptUtils::resolvePredefinedTime(
      name, internalClock, scriptBeginTime, sessionStartTime, lastAppCloseTime,
      lastResponseDuration, contextMinTime, contextMaxTime, contextStartTime,
      contextCreationTime, contextDeadline, contextNextRemind,
      lastLatenessMinutes, lastEarlyMinutes, contextNextAllow, lastFlagDuration,
      lastFlagExpiryTime);
...
void CyberDom::scheduleNextPopup() {
  ...
  QString lowerStatus = currentStatus.toLower();
  if (!scriptParser->getScriptData().statuses.contains(lowerStatus))
    return;

  const StatusDefinition &status =
      scriptParser->getScriptData().statuses.value(lowerStatus);
  const ScriptData &data = scriptParser->getScriptData();
  ...
  if (!status.popupGroup.isEmpty()) {
    ...
    if (data.popupGroups.contains(groupName)) {
      ...
      groupFound = true;
      // Note: If the group exists but has no interval set, we treat it as "no
      // popups for this group" unless we want to fallback to status. Usually
      // group overrides completely.
    }
  }
  ...
  if (intervalMin.isEmpty()) {
    qDebug() << "[Popup] No PopupInterval for status:" << currentStatus
             << (groupFound ? "(via Group)" : "");
    return;
  }

  QString range = intervalMin;
  if (!intervalMax.isEmpty())
    range += "," + intervalMax;
  ...
  if (delaySecs > 0) {
    ...
    qDebug() << "[Popup] Next popup scheduled in" << delaySecs << "seconds ("
...
void CyberDom::triggerPopup() {
  ...
  // Helper to convert simple QStringList to QList<QStringList> for
  // checkRequirements.
  ...
  if (!status.popupGroup.isEmpty()) {
    ...
    if (it != data.popupGroups.end()) {
      activeGroupDef =
          &it.value(); // Safe: points to the persistent object in the map
  ...
  if (activeGroupDef) {
    ...
    if (!checkRequirements(wrap(activeGroupDef->popupIfFlags),
                           wrap(activeGroupDef->noPopupIfFlags))) {
      contextAllowed = false;
    }
  } else {
    // Fallback to STATUS conditions
    if (!checkRequirements(wrap(status.popupIfFlags),
                           wrap(status.noPopupIfFlags))) {
      contextAllowed = false;
    }
  }
  ...
  for (const PopupDefinition &def : popups) {
    ...
    // Rule 1: If the Popup defines groups, the Current Status MUST belong to
    // one of them.
    ...
    // Rule 2: If the Active Group has "GroupOnly=1", the Popup MUST belong to
    // this group. (This filters out generic popups that have no group set)
    ...
    if (!def.preStatuses.isEmpty()) {
      ...
      for (const QString &ps : def.preStatuses) {
        if (ps.compare(currentStatus, Qt::CaseInsensitive) == 0) {
          match = true;
          break;
        }
      }
    ...
    // Flags
    if (!checkRequirements(wrap(def.ifFlags), wrap(def.notIfFlags)))
      continue;

    // Time
    if (!isTimeAllowed(def.notBeforeTimes, def.notAfterTimes,
                       def.notBetweenTimes))
      continue;
...
void CyberDom::executePopup(const QString &popupName) {
  ...
  for (const ScriptAction &action : def.actions) {
    ...
    switch (action.type) {
    case ScriptActionType::Message:
      QMessageBox::information(this, def.title.isEmpty() ? "Popup" : def.title,
                               replaceVariables(action.value));
      break;
    case ScriptActionType::ProcedureCall:
...
void CyberDom::exportClothes() {
  loadClothingInventory();

  if (clothingInventory.isEmpty()) {
    QMessageBox::information(this, "Export Clothes",
                             "No clothing items to export");
    return;
  }

  // Create the JSON Array from the inventory
  QJsonArray clothesArray;

  for (const ClothingItem &item : clothingInventory) {
    ...
    QMapIterator<QString, QString> i(item.attributes);
    while (i.hasNext()) {
      i.next();
      attrsObject[i.key()] = i.value();
    }
    jsonItem["attributes"] = attrsObject;

    // Checks
    QJsonArray checksArray;
    for (const QString &check : item.checks) {
      checksArray.append(check);
    }
    jsonItem["checks"] = checksArray;

    clothesArray.append(jsonItem);
  }

  // Prepare the File
  QString docsPath =
      QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation);
  QString dirPath = docsPath + "/CyberDom";

  QDir dir(dirPath);
  if (!dir.exists())
    dir.mkpath(".");

  QString filePath = dirPath + "/clothing_export.json";
  ...
  QJsonDocument saveDoc(clothesArray);
  saveFile.write(saveDoc.toJson(QJsonDocument::Indented));
  saveFile.close();

  QMessageBox::information(this, "Export Successful",
                           QString("Successfully exported %1 items to:\n%2")
                               .arg(clothesArray.size())
                               .arg(filePath));
}

void CyberDom::loadClothingInventory() {
  clothingInventory.clear();

  // Target the exact file path used in ReportClothing::saveClothingItems
  QDir dataDir(QDir::homePath() + "/.cyberdom");
  QString filePath = dataDir.absoluteFilePath("clothing.ini");

  if (!QFile::exists(filePath)) {
    qDebug() << "No saved clothing file found at:" << filePath;
    return;
  }

  QSettings settings(filePath, QSettings::IniFormat);

  // Read from the "items" array
  int size = settings.beginReadArray("items");
  for (int i = 0; i < size; ++i) {
    settings.setArrayIndex(i);

    QString jsonString = settings.value("data").toString();

    if (!jsonString.isEmpty()) {
      ClothingItem item = ClothingItem::fromString(jsonString);
  ...
  qDebug() << "Successfully loaded" << clothingInventory.size() << "items from"
           << filePath;
}

void CyberDom::importClothes() {
  QString filePath = QFileDialog::getOpenFileName(
      this, "Import Clothing List",
      QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation),
      "Clothing Files (*.json *.txt);;JSON Files (*.json);;Text Files (*.txt)");

  if (filePath.isEmpty())
    return;

  if (filePath.endsWith(".json", Qt::CaseInsensitive)) {
    parseAndMergeJson(filePath);
  } else if (filePath.endsWith(".txt", Qt::CaseInsensitive)) {
    parseAndMergeTxt(filePath);
  } else {
    QMessageBox::warning(this, "Import Error", "Unsupported file format.");
  }
}

void CyberDom::parseAndMergeJson(const QString &filePath) {
  QFile file(filePath);
  ...
  // Load current inventory first so we don't overwrite, we merge
  loadClothingInventory();

  // Helper to normalize types against the currently loaded script
  auto normalizeType = [&](QString rawType) -> QString {
    if (!scriptParser)
      return rawType;
    const auto &definedTypes = scriptParser->getScriptData().clothingTypes;
    if (definedTypes.contains(rawType))
      return rawType;
    for (auto it = definedTypes.begin(); it != definedTypes.end(); ++it) {
      if (it.key().compare(rawType, Qt::CaseInsensitive) == 0)
        return it.key();
    }
    return rawType;
  };
  ...
  for (const QJsonValue &val : arr) {
    ...
    // Reconstruct the item manually or use a helper if available
    ClothingItem item;
    item.name = obj["name"].toString();
    item.type = normalizeType(obj["type"].toString());
    // item.imagePath = obj["imagePath"].toString();

    // Attributes
    QJsonObject attrs = obj["attributes"].toObject();
    for (auto it = attrs.constBegin(); it != attrs.constEnd(); ++it) {
      item.attributes.insert(it.key(), it.value().toString());
    }

    // Checks
    QJsonArray checksArr = obj["checks"].toArray();
    for (const QJsonValue &c : checksArr)
      item.checks.append(c.toString());

    // // Tags
    // QJsonArray tagsArr = obj["tags"].toArray();
    // for (const QJsonValue &t : tagsArr) item.tags.insert(t.toString());

    // Prevent duplicates based on Name + Type
    bool exists = false;
    for (const auto &existing : clothingInventory) {
  ...
  if (addedCount > 0) {
    saveClothingInventory();
    QMessageBox::information(this, "Import Successful",
                             QString("Imported %1 new items.").arg(addedCount));
  } else {
    QMessageBox::information(this, "Import",
                             "No new items found (duplicates skipped).");
  }
}

void CyberDom::parseAndMergeTxt(const QString &filePath) {
  QFile file(filePath);
  ...
  // Load existing items first to merge
  loadClothingInventory();
  ...
  auto saveCurrentItem = [&]() {
    if (!currentItem.name.isEmpty()) {
      // Check duplicates
      bool exists = false;
      for (const auto &existing : clothingInventory) {
        if (existing.name == currentItem.name &&
      ...
      if (!exists) {
        clothingInventory.append(currentItem);
        addedCount++;
      }
    }
  };
  ...
  while (!in.atEnd()) {
    ...
    if (line.startsWith("[ClothItem-")) {
      // Save the previous item if one existed
      saveCurrentItem();
    ...
    if (eqIndex > -1) {
      ...
      } else if (key.compare("@@ClothType", Qt::CaseInsensitive) == 0) {
        currentItem.type = value;
      } else if (key.compare("items", Qt::CaseInsensitive) == 0) {
        continue;
      } else {
        currentItem.attributes.insert(key, value);
      }
    }
  }
  ...
  if (addedCount > 0) {
    ...
    QMessageBox::information(
        ...
        QString("Imported %1 new items from Text file.").arg(addedCount));
  } else {
    QMessageBox::information(this, "Import", "No new items found.");
  }
}

void CyberDom::saveClothingInventory() {
  // Write to the standard location: ~/.cyberdom/clothing.ini
  QDir dataDir(QDir::homePath() + "/.cyberdom");
  if (!dataDir.exists())
    dataDir.mkpath(".");

  QSettings settings(dataDir.absoluteFilePath("clothing.ini"),
                     QSettings::IniFormat);

  // We must mimic the structure used in Report Clothing:
  // An array named "items" containing a "data" key with the JSON string.
  settings.beginWriteArray("items", clothingInventory.size());
  for (int i = 0; i < clothingInventory.size(); ++i) {
    settings.setArrayIndex(i);
    settings.setValue("data", clothingInventory[i].toString());
  }
  settings.endArray();
  settings.sync();
