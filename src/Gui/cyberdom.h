...
// Define FlagData structure outside the class so it can be used by other
// classes
struct FlagData {
  QString name;
  QDateTime setTime;
...
#define TOSTRING(x) STRINGIFY(x)
  ...
  QString
  replaceVariables(const QString &input, const QString &contextName = "",
                   const QString &contextTitle = "", int contextSeverity = 0,
  ...
  QStringList getAssignmentResources(const QString &name,
                                     bool isPunishment) const;
  QSet<QString> getResourcesInUse() const;
  bool hasActiveBlockingPunishment() const;

  const QMap<QString, QMap<QString, QString>> &getIniData() const {
    return iniData;
  }

  QStringList getAvailableJobs();
  QMap<QString, QDateTime> getJobDeadlines() const { return jobDeadlines; }
  int getPunishmentAmount(const QString &name) const {
    return punishmentAmounts.value(name, 0);
  }
  ...
  bool startAssignment(const QString &assignmentName, bool isPunishment,
  ...
  void removeJobDeadline(const QString &jobName) {
  ...
  // Clothing management
  QString getCurrentClothingInstructions() const {
    return lastClothingInstructions;
  }
  QString getClothingReportPrompt() const;
  void processClothingReport(const QList<ClothingItem> &wearingItems,
                             bool isNaked);
  ...
  const QList<ClothingItem> &getClothingInventory() const {
    return clothingInventory;
  }
  void addClothingItem(const ClothingItem &item);
  QMap<QDate, QStringList> getHolidays() const;

  QString getAssignmentEstimate(const QString &assignmentName,
                                bool isPunishment) const;
  ...
  // Instructions
  QString resolveInstruction(const QString &name,
                             QStringList *chosenItems = nullptr,
                             QSet<QString> visited = QSet<QString>(),
                             bool isClothingContext = false);
...
private:
  ...
  QString getIniValue(const QString &section, const QString &key,
  ...
  getPunishmentDefinition(const QString &instanceName) const;
  ...
  bool itemMatchesCheck(const ClothingItem &item,
  ...
  void executePunish(const QString &severityValue, const QString &message,
                     const QString &group);
  ...
  bool isTimeAllowed(const QStringList &notBefore, const QStringList &notAfter,
  ...
  bool checkRequirements(const QList<QStringList> &ifGroups,
  ...
  void triggerPointCamera(const QString &text, const QString &minInterval,
  ...
  QVariant
  getTimeVariableValue(const QString &name, const QString &contextMinTime = "",
                       const QString &contextMaxTime = "",
                       const QDateTime &contextStartTime = QDateTime(),
                       const QDateTime &contextCreationTime = QDateTime(),
                       const QDateTime &contextDeadline = QDateTime(),
...
private slots:
  ...
  void onImageCaptureError(int id, QImageCapture::Error error,
