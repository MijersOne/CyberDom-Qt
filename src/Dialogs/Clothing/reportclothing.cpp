...
ReportClothing::ReportClothing(QWidget *parent, ScriptParser *parser,
                               bool forced, const QString &customTitle)
    : QDialog(parent), ui(new Ui::ReportClothing), parser(parser),
      isForced(forced) {
  ui->setupUi(this);
  ...
  connect(ui->btn_AddType, &QPushButton::clicked, this,
          &ReportClothing::openAddClothTypeDialog);
  connect(ui->btn_Cancel, &QPushButton::clicked, this,
          &ReportClothing::cancelDialog);
  connect(ui->btn_Remove, &QPushButton::clicked, this,
          &ReportClothing::removeItemFromWearing);
  connect(ui->btn_Report, &QPushButton::clicked, this,
          &ReportClothing::submitReport);
  connect(ui->btn_Select, &QPushButton::clicked, this,
          &ReportClothing::addItemToWearing);
  connect(ui->btn_Add, &QPushButton::clicked, this,
          &ReportClothing::openAddClothingDialog);
  connect(ui->btn_Edit, &QPushButton::clicked, this,
          &ReportClothing::editSelectedItem);
  connect(ui->btn_Delete, &QPushButton::clicked, this,
          &ReportClothing::deleteSelectedItem);

  connect(ui->cb_Type, &QComboBox::currentTextChanged, this,
          &ReportClothing::onTypeSelected);
  connect(ui->chk_Naked, &QCheckBox::toggled, this,
          &ReportClothing::onNakedCheckboxToggled);

  connect(ui->lst_Wearing, &QListWidget::itemSelectionChanged, this,
          &ReportClothing::onWearingItemSelected);
  connect(ui->lst_Available, &QListWidget::clicked, this,
          &ReportClothing::onAvailableItemSelected);

  connect(ui->lst_Available, &QListWidget::itemDoubleClicked, this,
          &ReportClothing::addItemToWearing);
  connect(ui->lst_Wearing, &QListWidget::itemDoubleClicked, this,
          &ReportClothing::removeItemFromWearing);
  ...
  if (ui->cb_Type->count() == 0) {
    ui->cb_Type->addItems(
        {"Top", "Bottom", "Underwear", "Shoes", "Accessories"});
...
void ReportClothing::loadClothingItems() {
  ...
  QMap<QString, QString> canonicalTypeMap;
  if (parser) {
    const auto &definedTypes = parser->getScriptData().clothingTypes;
    for (auto it = definedTypes.begin(); it != definedTypes.end(); ++it) {
      // Key is lowercase, Value is the Actual Name defined in script
      canonicalTypeMap.insert(it.key().toLower(), it.key());
    }
  }
  ...
  for (int i = 0; i < itemCount; i++) {
    ...
    // Normalize the Type
    QString rawType = item.getType();
    QString lowerType = rawType.toLower();

    if (canonicalTypeMap.contains(lowerType)) {
      // Found a match! Use the script's official spelling
      QString canonicalName = canonicalTypeMap.value(lowerType);

      if (rawType != canonicalName) {
        item.setType(canonicalName);
      }

      clothingByType[canonicalName].append(item);
    } else {
      clothingByType[rawType].append(item);
    }
  }
  settings.endArray();
  ...
  for (int i = 0; i < wearingCount; ++i) {
    ...
    // Normalize wearing items too, just in case
    if (parser) {
      QString lower = item.getType().toLower();
      if (canonicalTypeMap.contains(lower)) {
        item.setType(canonicalTypeMap.value(lower));
      }
    }
...
void ReportClothing::saveClothingItems() {
  ...
  // Remove existing "items" group to clear out stale data
...
void ReportClothing::updateWearingItems() {
  ...
  for (const ClothingItem &item : wearingItems) {
    ui->lst_Wearing->addItem(
        QString("%1 (%2)").arg(item.getName()).arg(item.getType()));
  }
}
...
void ReportClothing::onTypeSelected(const QString &type) {
  ...
  QList<ClothingAttribute> attributes =
      clothTypeAttributes.value(type.toLower());
...
void ReportClothing::removeItemFromWearing() {
  ...
  if (row < 0) {
    QMessageBox::warning(this, "Selection Required",
                         "Please select an item from the wearing list.");
    return;
  }
...
void ReportClothing::openAddClothTypeDialog() {
  ...
  connect(&addClothTypeDialog, &AddClothType::clothTypeAdded, this,
          &ReportClothing::addClothTypeToComboBox);
...
void ReportClothing::openAddClothingDialog() {
  ...
  QList<ClothingAttribute> attributes =
      clothTypeAttributes.value(selectedType.toLower());
  AddClothing addClothingDialog(this, selectedType, attributes);

  // Use UniqueConnection to prevent duplicate signal processing
  connect(&addClothingDialog, &AddClothing::clothingItemAdded, this,
          static_cast<void (ReportClothing::*)(const ClothingItem &)>(
              &ReportClothing::addClothingItemToList),
          Qt::UniqueConnection);
...
void ReportClothing::resolveItemChecks(ClothingItem &item) {
  ...
  if (clothingTypes.contains(typeName)) {
    ...
    for (auto it = attrs.constBegin(); it != attrs.constEnd(); ++it) {
      ...
      for (auto vcIt = def.valueChecks.constBegin();
           vcIt != def.valueChecks.constEnd(); ++vcIt) {
    ...
    qDebug() << "[ReportClothing] Resolved checks for" << item.getName() << ":"
             << resolvedChecks;
  }
}
...
void ReportClothing::addClothingItemToList(const ClothingItem &item) {
  ...
  if (itemName.isEmpty() || itemType.isEmpty()) {
    QMessageBox::warning(this, "Invalid Input",
                         "Clothing name and type must not be empty.");
    return;
  }

  // Normalize type casing (Title Case) to ensure it matches the map keys
  if (!itemType.isEmpty())
    itemType[0] = itemType[0].toUpper();
  ...
  for (const ClothingItem &existing : items) {
    if (existing.getName().trimmed().compare(itemName, Qt::CaseInsensitive) ==
            0 &&
        existing.getType().trimmed().compare(itemType, Qt::CaseInsensitive) ==
            0) {

      QMessageBox::warning(this, "Duplicate Item",
                           QString("An item named '%1' already exists in '%2'. "
  ...
  normalizedItem.setType(itemType); // Assuming setter exists, otherwise reuse
                                    // item logic is fine if type matches
...
void ReportClothing::editSelectedItem() {
  ...
  for (int i = 0; i < clothingByType[selectedType].size(); i++) {
    if (clothingByType[selectedType][i].getName() == selectedItemName) {
      ...
      AddClothing editDialog(this, selectedType,
                             clothingByType[selectedType][i], attributes);
      ...
      connect(&editDialog, &AddClothing::clothingItemEdited, this,
              [this, i, selectedType](const ClothingItem &editedItem) {
                ...
                for (int j = 0; j < clothingByType[editedType].size(); ++j) {
                  if (j == i && editedType == selectedType)
                    continue; // Skip self

                  const ClothingItem &existing = clothingByType[editedType][j];
                  if (existing.getName().compare(editedName,
                                                 Qt::CaseInsensitive) == 0 &&
                      existing.getType().compare(editedType,
                                                 Qt::CaseInsensitive) == 0) {

                    QMessageBox::warning(
                        this, "Duplicate Name",
                        QString("An item named '%1' already exists in '%2'.")
                            .arg(editedName, editedType));
                    return;
                  }
                }
...
bool ReportClothing::isNaked() const { return ui->chk_Naked->isChecked(); }
...
ReportClothing::~ReportClothing() { delete ui; }
